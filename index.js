...
const buildItems = (items, result = []) => {
  for (let i = 0; i < items.length; i++) {
    ...
    const itemPubDate = item.getElementsByTagName("dc:date")[0].textContent;
    const itemComments = item.getElementsByTagName("link")[0].textContent;
...
const handleRSS = (RSS, title, link, jsonOutputPath) => {
  ...
  const items = buildItems(parsedXML.getElementsByTagName("item"));

  writeIndexJson(items, jsonOutputPath);
  logReadmeMdContents(title, link, items);
};

const title = "Soylent News";
const link = "https://kherrick.github.io/soylent-news/";
const jsonOutputPath = "./index.json";

handleRSS(
  await (await fetch("https://soylentnews.org/index.rss")).text(),
  title,
  link,
  jsonOutputPath
);
